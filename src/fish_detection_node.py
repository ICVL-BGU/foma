...
class FishDetectionNode(AbstractNode):
    ...
    def process_image(self):
        ...
        if kps.shape[1] == 0:
            ...
            if do_log:
                self.loginfo("Fish not detected")
            ...
            confs = [round(float(c), 3) for c in prediction.boxes.conf]
            if do_log:
                self.loginfo(f"Fish detections: {len(confs)}, confs: {confs}")
            for kp in kps:
