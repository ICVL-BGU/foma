...
class GoHomeNode(AbstractNode):
    ...
    def _compute_center_error(self, ranges_m):
        ...
        """
        d_fwd   = self._get_dist(ranges_m, 0)    # forward wall
        d_bwd   = self._get_dist(ranges_m, 180)  # backward wall
        d_left  = self._get_dist(ranges_m, 90)   # left wall
        d_right = self._get_dist(ranges_m, 270)  # right wall

        if None in (d_fwd, d_bwd, d_left, d_right):
